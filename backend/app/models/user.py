...
class User(Base):
	...
	# Персональная информация (совместимость)
	...
	__table_args__ = (
		...
		CheckConstraint('length(hashed_password) >= 10', name='ck_users_password_hash_min_length'),
	...
	def full_name_or_username(self) -> str:
		"""Удобное имя пользователя для отображения."""
		if self.full_name:
